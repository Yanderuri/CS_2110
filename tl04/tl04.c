...
{
    if (question == NULL || strlen(question) == 0 || strcmp(question, "") == 0)
    {
        return FAILURE;
    }
    ...
    newPost->question = (char *)malloc(sizeof(char) * (len + 1));
    if (newPost->question == NULL)
    {
        free(newPost);
        return FAILURE;
    }
    strncpy(newPost->question, question, len);
    // Ensure last char is a null terminator
    newPost->question[len] = '\0';
    newPost->category = category;
    if (numPosts == capacity)
    {
        capacity *= 2;
        struct post_t ** old_stack_arr = stack_arr;
        // Reallocate the stack with doubled the capacity
        stack_arr = (struct post_t **) realloc(stack_arr, sizeof(struct post_t *) * capacity);
        if (stack_arr == NULL)
        {
            stack_arr = old_stack_arr;
            free(newPost->question);
            free(newPost);
            return FAILURE;
        }
    }
...
{
    if (data == NULL || numPosts == 0)
    {
        return FAILURE;
    }
    struct post_t *answer = stack_arr[numPosts - 1];
...
int destroy_stack(void)
{
    while (numPosts > 0)
    {
        free(stack_arr[numPosts - 1]->question);
        free(stack_arr[numPosts - 1]);
        stack_arr[numPosts - 1] = NULL;
        numPosts--;
    }
    numPosts = 0;
